	...
	else
	{
		...
		if(pxDescriptor->xDataLength != 0 && pxDescriptor->pucEthernetBuffer != NULL)
		{
			...
			if(pxDescriptor->xDataLength < MIN_ETHERNET_PACKET_SIZE)
			{
				...
				/* Buffer pointer az eredeti csomag sz�m�ra*/
				...
				/* Nem �ll�tjuk be a EOP bitet, az ethernet packet t�bb descriptor-on kereszt�l h�z�dik */
				...
				/* Tov�bbl�p�nk a k�vetkez� descriptor-ra ami a csomag padding-ot fogja v�gezni */
				pxTransmitBufferDescriptor++;
				/* Buffer pointer a padding sz�m�ra */
...
//TODO: ennek a megszak�t�snak kell felszabad�tani a elk�ld�tt BD-khez tartoz� network puffereket
...
#error ipconfigZERO_COPY_TX_DRIVER not available yet
	...
	/* Nyugt�zzuk az EMAC control modul RX megszak�t�s�t. */
...
void dp83630_set_page(hdkif_t *hdkif, uint16_t reg){
	...
	MDIOPhyRegWrite(hdkif->mdio_base, hdkif->phy_addr, PAGESEL_REG, page);
}
...
static void prvEmacDMAInit(hdkif_t *hdkif)
{
      ...
      for(i = 0; i < EMAC_TXDMA_PBUF_ALLOC; i++)
      {
    	  if (i < (EMAC_TXDMA_PBUF_ALLOC - 1))pxCurrentBD->next = (emac_rx_bd_t *)EMACSwizzleData((uint32)(pxCurrentBD + 1));
        	  ...
        	  {
        	  pxCurrentBD->next = (emac_rx_bd_t *)NULL;	/* l�ncolt lista v�ge */
        	  }
